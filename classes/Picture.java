...
public class Picture extends SimplePicture 
{
  ...
  public void fixUnderwater() {
    ...
    for (Pixel[] rowArray: pixels) {
      for (Pixel pixelObj: rowArray) {
        ...
        if (pixelObj.getRed() < 24 && 
            pixelObj.getGreen() > 150 &&
            pixelObj.getBlue() > 150) 
  ...
  /** Method that mirrors the picture around a 
    * hoizontal mirror in the center of the picture
    * from bottom to top */
  public void mirrorHorizontalInv()
  {
    Pixel[][] pixels = this.getPixels2D();
    Pixel topPixel = null;
    Pixel bottomPixel = null;
  ...
  /** Method to show large changes in color !2!
    * @param edgeDist the distance for finding edges
    */
  public void edgeDetection2(int edgeDist)
  {
    ...
    for (int row = 0; row < pixels.length; row++)
    {
           ...
           col < pixels[0].length-1; col++)
      {
        ...
        if (row < pixels.length-1) bottomPixel = pixels[row+1][col];
        else bottomPixel = leftPixel;
